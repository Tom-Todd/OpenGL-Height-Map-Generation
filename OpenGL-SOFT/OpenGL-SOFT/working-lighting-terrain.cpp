...
Terrain::Terrain(GLuint shadersID) {
	//GLuint programID = LoadShaders("terrainVertex.glsl", "terrainFragment.glsl");
	GLuint programID = LoadShaders("vertex.glsl", "fragment.glsl");
	MatrixID = glGetUniformLocation(programID, "MVP");
	setShaders(programID);
	Texture = loadBMP("grass.bmp");
	...
	myNoise.SetSeed((static_cast <float> (rand()) / static_cast <float> (RAND_MAX))*100);
	...
	for (int i = 0; i < w-1; i++) {
		for (int j = 0; j < h; j++) {
			glm::vec3 vertex = glm::vec3(j*size, heightMap[j][i], i*size);
			uvR[i][j] = glm::vec2(u,v);
...
void Terrain::render(glm::mat4 * Projection, Camera camera) {
	...
	glUniform3f(LightI, 100, 100, 100);
